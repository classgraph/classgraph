...
/**
 * Tests that an enum constant with a constant-specific class body is serialized
 * as its name, like any other enum constant.
 *
 * <p>
 * Such a constant is an instance of an anonymous subclass of the enum type, and
 * {@link Class#isEnum()} is false for that subclass. The serializer used to
 * test {@code value.getClass().isEnum()}, so these constants were not
 * recognized as enum values, and were serialized as an (empty) object rather
 * than as a quoted name -- which then failed to deserialize at all.
 */
public class EnumWithConstantBodyTest {
    ...
    public void enumWithConstantBodyIsSerializedAsItsName() {
        final var json = JSONSerializer.serializeObject(new EnumHolder(), 0, false);
    ...
    public void enumWithConstantBodyRoundTrips() {
        final var holder = new EnumHolder();
        ...
        final var deserialized = JSONDeserializer.deserializeObject(EnumHolder.class, json);
