...
/**
 * Tests that a map whose keys are not {@link Comparable} is serialized with
 * each key still paired with its own value.
 *
 * <p>
 * Map keys are sorted so that the serialized form is deterministic. When the
 * keys are {@link Comparable} they are sorted before the values are looked up,
 * so keys and values stay in step. When they are not (a {@code Class<?>} key is
 * a legal map key, but {@link Class} does not implement {@link Comparable}),
 * the serializer used to sort the array of stringified keys on its own, after
 * the values had already been collected in the map's own iteration order -- so
 * each key ended up paired with a different key's value.
 */
public class JSONSerializerMapKeyOrderTest {
    /**
     * A class with a map field keyed by {@code Class<?>}, i.e. by a non-Comparable
     * basic value type.
     */
    ...
    /**
     * Each key must be paired with its own value, and the keys must still come out
     * in sorted order.
     */
    @Test
    public void nonComparableMapKeysStayPairedWithTheirValues() {
        final var holder = new ClassKeyedMapHolder();
        // Insert in an order whose stringified form is not already sorted, so that
        ...
        final var json = JSONSerializer.serializeObject(holder, 0, false);
    ...
    /**
     * A class with a map field keyed by String, i.e. by a Comparable basic value
     * type.
     */
    ...
    public void comparableMapKeysAreStillSortedAndPaired() {
        final var holder = new StringKeyedMapHolder();
