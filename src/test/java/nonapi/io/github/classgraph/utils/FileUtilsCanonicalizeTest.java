...
public class FileUtilsCanonicalizeTest {
    ...
    public void aParentSegmentAfterASymlinkIsResolvedByThePlatform(@TempDir final Path tempDir) throws IOException {
        final Path realDir = Files.createDirectory(tempDir.resolve("real"));
        final Path innerDir = Files.createDirectory(realDir.resolve("inner"));
        final Path linkedDir = createSymbolicLinkOrSkip(tempDir.resolve("link"), innerDir);

        // Whatever this platform says "link/.." is, canonicalize() has to say the same
        final Path parentViaLink = linkedDir.resolve("..").toRealPath();
