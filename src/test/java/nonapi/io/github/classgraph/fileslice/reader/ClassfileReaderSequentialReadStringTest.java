 ...
 *
 * <p>
 * A {@link ClassfileReader} built on an {@link java.io.InputStream} starts with an allocated but completely unfilled
 * buffer, so a read that does not first grow the buffer returns the zero bytes the array was allocated with, rather
 * than the stream contents. Every other sequential read method delegates to its random access counterpart, which
 * buffers the requested range first; {@code readString} used to read straight out of the buffer instead.
 */
public class ClassfileReaderSequentialReadStringTest {
    ...
    public void sequentialReadStringBuffersBeforeReading() throws IOException {
        ...
        try (ClassfileReader reader = new ClassfileReader(new ByteArrayInputStream(data), null)) {
    ...
    public void sequentialReadStringInterleavesWithOtherReads() throws IOException {
        final byte[] data = new byte[] { 0, 3, 'a', 'b', 'c' };
        try (ClassfileReader reader = new ClassfileReader(new ByteArrayInputStream(data), null)) {
            final int len = reader.readUnsignedShort();
