 ...
 * checks unsatisfiable for a conforming central directory entry, so the times were never read, and the entry's
 * last modified time always fell back to the MS-DOS timestamp, which has a resolution of only two seconds.
 *
 * <p>
...
public class ExtendedTimestampExtraFieldTest {
    ...
    private static long lastModifiedTimeReadBack(final File tempDir, final String jarName, final byte[] extraField)
            throws Exception {
        final byte[] nameBytes = ENTRY_NAME.getBytes(StandardCharsets.UTF_8);
        ...
        try {
            final Entry<LogicalZipFile, String> logicalZipFileAndPackageRoot = //
                    nestedJarHandler.nestedPathToLogicalZipFileAndPackageRootMap.get(jarFile.getPath(),
                            /* log = */ null);
            final List<FastZipEntry> entries = logicalZipFileAndPackageRoot.getKey().entries;
