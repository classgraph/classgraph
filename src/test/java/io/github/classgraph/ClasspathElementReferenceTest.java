 ...
 * nondeterministic, so both the classpath ordering key and the classloader must be taken from the winning reference,
 * not from whichever work unit happened to create the element.
 *
 * <p>
 * Otherwise the classpath order is nondeterministic (#810), and {@link ClassInfo#loadClass()} intermittently loads a
 * class through the wrong classloader (which is what made
 * {@code io.github.classgraph.issues.issue267.ClassLoadingWorksWithParentLastLoadersStubTest} fail intermittently on
 * macOS and Windows). The end-to-end races are timing-dependent and cannot be forced, so the precedence rule that
 * they depend on is tested directly here.
 */
