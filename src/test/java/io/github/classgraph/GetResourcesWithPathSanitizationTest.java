 ...
 * {@link ScanResult#getResourcesWithPathIgnoringAccept(String)} sanitize their argument without throwing, even for a
 * path that normalizes away to nothing.
 */
public class GetResourcesWithPathSanitizationTest {
    ...
    public void pathNormalizingToRootMatchesNothing() {
        try (ScanResult scanResult = new ClassGraph().acceptPackages(GetResourcesWithPathSanitizationTest.class
                .getPackage().getName()).scan()) {
