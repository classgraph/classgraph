...
public class Issue83Test {
    ...
    public void jarWhitelist() {
        ...
        final ResourceList resources = new ClassGraph().overrideClasspath(jarPathURL)
                .whitelistJars("nested-jars-level1.zip").scan().getAllResources();
    ...
    public void jarBlacklist() {
        ...
        final ResourceList resources = new ClassGraph().overrideClasspath(jarPathURL)
                .blacklistJars("nested-jars-level1.zip").scan().getAllResources();
        for (final Resource res : resources) {
            paths.add(res.getPath());
        }
