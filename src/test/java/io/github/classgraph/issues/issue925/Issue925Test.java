...
public class Issue925Test {
    ...
    public static void buildWar(@TempDir final File tempDir) throws IOException {
        final String classfilePath = Widget.class.getName().replace('.', '/') + ".class";
        final String libClassfilePath = LibWidget.class.getName().replace('.', '/') + ".class";

        // WEB-INF/lib/mylib.jar
        final ByteArrayOutputStream libJar = new ByteArrayOutputStream();
        try (ZipOutputStream zipOut = new ZipOutputStream(libJar)) {
        ...
        try (ZipOutputStream zipOut = new ZipOutputStream(new java.io.FileOutputStream(war))) {
    ...
    private static void copyClassfile(final String classfilePath, final OutputStream out) throws IOException {
        try (InputStream in = Issue925Test.class.getClassLoader().getResourceAsStream(classfilePath)) {
    ...
    public void warUrlsAreResolvedToJarPaths() {
        ...
        final String expected = FastPathResolver.resolve(warUrl) + "!/WEB-INF/classes";
    ...
    public void warUrlClasspathElementIsScanned() {
        try (ScanResult scanResult = new ClassGraph()
                .overrideClasspath("war:" + warUrl + "*/WEB-INF/classes/").enableClassInfo().scan()) {
            // Both classes are found, not just the one in WEB-INF/classes/, because ClassGraph already treats
            // "WEB-INF/classes/" as a package root and "WEB-INF/lib/" as a library directory within a WAR file,
            // so the whole webapp is scanned. That matches what the webapp's classloader can load.
            assertThat(scanResult.getAllClasses().getNames()).containsExactlyInAnyOrder(Widget.class.getName(),
                    LibWidget.class.getName());
        }
        try (ScanResult scanResult = new ClassGraph()
                .overrideClasspath("war:" + warUrl + "*/WEB-INF/lib/mylib.jar").enableClassInfo().scan()) {
    ...
    public void warFileClasspathElementIsScanned() {
        try (ScanResult scanResult = new ClassGraph().overrideClasspath(war.getPath()).enableClassInfo().scan()) {
            assertThat(scanResult.getAllClasses().getNames()).containsExactlyInAnyOrder(Widget.class.getName(),
                    LibWidget.class.getName());
        }
    }
