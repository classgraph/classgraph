...
/**
 * {@link ClassInfo#toStringWithSimpleNames()} rendered a generic class by delegating to
 * {@link ClassTypeSignature}, passing the already-simplified class name but {@code useSimpleNames = false}, so the
 * type parameter bounds, superclass and superinterfaces were left fully qualified. The non-generic branch of
...
public class GenericClassSimpleNamesTest {
    ...
    public void genericClassUsesSimpleNamesThroughout() {
        try (ScanResult scanResult = new ClassGraph()
            ...
            assertThat(generic.toString())
                    .isEqualTo("public static class " + Generic.class.getName()
                            + "<T extends java.lang.Number> extends java.util.ArrayList<java.lang.String> "
                            + "implements java.util.List<java.lang.String>");

            final ClassInfo nonGeneric = scanResult.getClassInfo(NonGeneric.class.getName());
