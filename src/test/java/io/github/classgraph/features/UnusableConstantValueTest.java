...
public class UnusableConstantValueTest {
    ...
    /**
     * Generate a class with three fields: a static int field with the constant value 5, a non-static
     * {@code Object} field whose {@code ConstantValue} attribute holds an int, and a non-static int field whose
    ...
    public void theJvmLoadsTheClass() throws Exception {
        final Class<?> cls = new ByteArrayClassLoader().define(classfileBytes());
    ...
    public void anUnusableConstantValueIsIgnored(@TempDir final File tempDir) throws IOException {
        ...
        try (ScanResult scanResult = new ClassGraph().overrideClasspath(tempDir).enableFieldInfo()
                .enableStaticFinalFieldConstantInitializerValues().scan()) {
            final ClassInfo classInfo = scanResult.getClassInfo(CLASS_NAME);
