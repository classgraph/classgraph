...
public class NestedJarClasspathElementFileTest {
    ...
    private static byte[] jarWithResource(final String resourcePath) throws IOException {
        ...
        try (var zipOut = new ZipOutputStream(bytes)) {
            zipOut.putNextEntry(new ZipEntry(resourcePath));
            zipOut.write(resourcePath.getBytes());
            zipOut.closeEntry();
        }
    ...
    public void storedAndDeflatedNestedJarfilesReportTheOutermostJarfile(@TempDir final File tempDir)
            throws IOException {
        final var outerJarFile = new File(tempDir, "app.jar");
        final var storedJar = jarWithResource("stored.txt");
        try (var zipOut = new ZipOutputStream(new FileOutputStream(outerJarFile))) {
            final var storedEntry = new ZipEntry("lib/stored.jar");
        ...
        System.out.println("PROBE " + new ClassGraph()
                .enableClasspathEntries(outerJarFile + "!/lib/stored.jar", outerJarFile + "!/lib/deflated.jar")
                .getClasspathFiles());
        try (var scanResult = new ClassGraph()
                .enableClasspathEntries(outerJarFile + "!/lib/stored.jar", outerJarFile + "!/lib/deflated.jar")
                .scan()) {
            final var expected = outerJarFile.getCanonicalFile();
        ...
        assertThat(new ClassGraph()
                .enableClasspathEntries(outerJarFile + "!/lib/stored.jar", outerJarFile + "!/lib/deflated.jar")
                .getClasspathFiles()).containsExactly(outerJarFile.getCanonicalFile());
    }
}
