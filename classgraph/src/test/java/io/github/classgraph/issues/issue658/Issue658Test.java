...
public class Issue658Test {
    ...
    /** A system module that is accepted by name is scanned. */
    @Test
    public void anAcceptedSystemModuleIsScanned() {
        try (var scanResult = new ClassGraph().enableSystemModules().enableClassInfo().acceptModules("jdk.compiler")
    ...
    /**
     * Accepting a module by name does not enable a module source: the module is only scanned if the modules were
     * enabled too.
     */
    @Test
    public void acceptingAModuleByNameDoesNotEnableAModuleSource() {
        try (var scanResult = new ClassGraph().enableClasspath().enableClassInfo().acceptModules("jdk.compiler")
                .acceptPackages("com.sun.source.tree").scan()) {
